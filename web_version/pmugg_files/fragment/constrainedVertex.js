...
ms.constrainedVertex = function(vertex, settings) {
	this.vertex = vertex;
	this.row = -1;
...
ms.constrainedVertex.prototype.addChild = function(child) {
	this.children.push(child);
};

ms.constrainedVertex.prototype.getParent = function() {
	return this.parent;
};

ms.constrainedVertex.prototype.getParentOffset = function() {
	return this.parentOffset;
};

ms.constrainedVertex.prototype.setParent = function(parent, parentOffset) {
...
ms.constrainedVertex.prototype.print = function() {
	this.vertex.print();
};

ms.constrainedVertex.prototype.addLinearConstraint = function(constraint) {
...
ms.constrainedVertex.parallelDet = 1e-6;
	
ms.constrainedVertex.prototype.origin = function() {
	return ms.transistor.origin(this.dims);
};
	
ms.constrainedVertex.prototype.getM = function() {
...
ms.constrainedVertex.prototype.getB = function() {
	if (this.fixedB) {
		return this.fixedB;
	}
...
ms.constrainedVertex.prototype.setRowM = function(rowM) {
	var M = this.settings.M;
...
ms.constrainedVertex.prototype.setRowB = function(rowB) {
...
ms.constrainedVertex.prototype.setRow = function(row) {
	this.row = row;
...
// has been set.
ms.constrainedVertex.prototype.propagate = function() {
	var endpoints = this.vertex.getEndpoints();
	for (var i = 0; i < endpoints.length; i++) {
		var success = this.propagateEndpoint(endpoints[i]);
...
ms.constrainedVertex.hasEndpoint = function(constraints, endpoint) {
	return constraints.some(function(constraint) {
		return constraint && constraint.endpoint == endpoint;
	});
};

ms.constrainedVertex.prototype.propagateEndpoint = function(endpoint) {
	// For rigid vertices, they are constrained using the parent and children.
	if (!ms.transistor.isExtendable(endpoint.getEdge())) {
		return true;
	}
	var twin = endpoint.twin();
	var constraints = this.parent ? this.parent.constraints : this.constraints;
	if (ms.constrainedVertex.hasEndpoint(constraints, twin)) {
		return true;
	}
	var twinVertex = endpoint.twin().getVertex();
	return this.settings.getConstrained(twinVertex).addConstraint(endpoint, this.origin());
};

ms.constrainedVertex.prototype.fixVertex = function(b) {
	if (this.parent) {
...
ms.constrainedVertex.prototype.fixVertexBase = function(b) {
	this.row = -1;
	this.rowIndices = null;
	this.fixedB = b;
};

ms.constrainedVertex.prototype.fixVertexSoft = function(b) {
	if (this.parent) {
...
ms.constrainedVertex.prototype.addConstraint = function(endpoint, offset) {
	if (this.parent) {
		var pOffset = this.parentOffset.copy().scale(-1);
		return this.parent.addConstraint(endpoint, pOffset);
	}
	if (ms.constrainedVertex.hasEndpoint(this.constraints, endpoint)) {
		return true;
	}
	this.constraints.push({endpoint: endpoint, offset: offset});
	var num = this.constraints.length;
	if (num == 1) {
		this.settings.singleConstraintList.push(this);
		var u0 = this.endpointDir(endpoint);
		var twin = endpoint.twin();
		var endpoints = this.vertex.endpoints;
		for (var i = 0; i < endpoints.length; i++) {
			...
			if (Math.abs(mathG.det(U)) < ms.constrainedVertex.parallelDet) {
				...
				var prevVertex = this.settings.getConstrained(endpoint.vertex);
				var b0 = prevVertex.getB();
				b0 = mathG.add(b0, offset.toMatrix());
				this.setRowB(b0);
				this.setRowM(prevVertex.getM());
				
				var success = this.propagateEndpoint(endpoint1);
				if (!success) { return false; }
				this.addLinearConstraint({endpoint: endpoint1.twin(), offset: this.origin()});
			}
		}
	} else if (num == 2) {
		ms.remove(this, this.settings.singleConstraintList);
...
ms.constrainedVertex.prototype.updateEdgeLength = function(constraint) {
	// TODO: Adjust using the offset.
	var endpoint = constraint.endpoint;
	...
	var M = this.settings.M;
	var b = this.settings.b;
	var vertex1 = this.settings.getConstrained(endpoint.vertex);
	...
	if (vertex1.constraints.length >= 2) {		
		...
		var v = this.endpointDir(endpoint);
		...
		vertex1.addLinearConstraint({endpoint: constraint.endpoint.twin(), offset: this.origin()});
	}
};

ms.constrainedVertex.dot = function(a, b) {
...
ms.constrainedVertex.prototype.emptyM = function() {
	if (this.fixedB) {
		return true;
	}
	var M = this.getM();
...
ms.constrainedVertex.prototype.satisfiesConstraints = function(constraints) {	
	...
	for (var i = 0; i < constraints.length; i++) {
		...
		var endpoint = constraints[i].endpoint;
		var u = this.endpointDir(endpoint);
		var v = mathG.matrix([[-u.y], [u.x]]);
		u = u.toMatrix();
		var vertex1 = this.settings.getConstrained(constraints[i].endpoint.vertex);

		// Ignore vertices that already include this constraint.
		if (!vertex1.fixedB && ms.constrainedVertex.hasEndpoint(vertex1.constraints, endpoint.twin())) {
			continue;
		}
		var b0 = vertex1.getB();
		var b1 = this.getB();
		var v0 = ms.constrainedVertex.dot(v, mathG.add(b0, constraints[i].offset.toMatrix()))
		var v1 = ms.constrainedVertex.dot(v, b1);
		...
		/* 
		var u0 = ms.constrainedVertex.dot(u, b0);
		var u1 = ms.constrainedVertex.dot(u, b1);
		if (u1 < u0 + lengthMin) {			
...
ms.constrainedVertex.prototype.edgeRow = function(endpoint) {
	return this.settings.getEdgeRow(endpoint.edge);
};

ms.constrainedVertex.prototype.endpointM = function(endpoint) {
	var vertex = this.settings.getConstrained(endpoint.getVertex());
	return vertex.getM();
};

ms.constrainedVertex.prototype.endpointB = function(endpoint) {
	var vertex = this.settings.getConstrained(endpoint.getVertex());
	return vertex.getB();
};

ms.constrainedVertex.prototype.endpointDir = function(endpoint) {
	...
	if (endpoint.edgeIndex == 1) {
		dir.scale(-1);
	}
	return dir;
};

ms.constrainedVertex.prototype.applySingle = function() {
	var endpoint = this.constraints[0].endpoint;
	var offset = this.constraints[0].offset;
	...
	this.setRowM(this.endpointM(endpoint));
	this.setRowB(mathG.add(this.endpointB(endpoint), offset.toMatrix()));
...
ms.constrainedVertex.prototype.applyDouble = function() {
	...
	var u0 = this.endpointDir(endpoint0);
	var u1 = this.endpointDir(endpoint1);
	var U;
	if (this.dims == 2) {		
		var U = mathG.matrix([
	...
	var M = this.settings.M;
	var b = this.settings.b;
	var m0 = this.endpointM(endpoint0);
	var m1 = this.endpointM(endpoint1);
	var b0 = mathG.add(this.endpointB(endpoint0), offset0.toMatrix());
	var b1 = mathG.add(this.endpointB(endpoint1), offset1.toMatrix());
...
ms.constrainedVertex.prototype.setRigidConstraint = function(vertexB, offsetB) {
	if (this.parent) {
