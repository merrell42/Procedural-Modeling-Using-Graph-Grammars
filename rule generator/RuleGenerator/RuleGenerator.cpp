...
void printBoundaryTuple(const vector<int>& values) {
	cout << "[";
...
int findSplicedMatchGroup(const vector<GraphGroup>& groups, int matchIndex) {
	for (int g = 0; g < (int)groups.size(); g++) {
		for (int index : groups[g].graphIndices[1]) {
			if (index == matchIndex) {
				return g;
			}
		}
...
void printMatchIndices(const vector<int>& indices) {
	cout << "[";
...
void debugV15V33Grouping(
	const vector<GraphGroup>& groups,
	const vector<vector<vector<int>>>& allBoundaryValues,
	const vector<TemplateMatcher>& matchers,
	const char* stage
) {
	const auto& bentBoundaryValues = allBoundaryValues[0];
	const auto& splicedBoundaryValues = allBoundaryValues[1];
	const TemplateMatcher& splicedMatcher = matchers[1];

	for (int m = 0; m < (int)splicedMatcher.vertexValues.size(); m++) {
		if (!splicedMatcher.isV15V33Match(m)) {
			continue;
		}
		cout << "    grouping (" << stage << "): v15/v33 spliced match " << m
			<< " types at 4,5 = "
			<< splicedMatcher.matchVertexTypeValue(m, 4) << ","
			<< splicedMatcher.matchVertexTypeValue(m, 5)
			<< " boundary tuple ";
		printBoundaryTuple(splicedBoundaryValues[m]);
		cout << "\n";

		int groupIndex = findSplicedMatchGroup(groups, m);
		if (groupIndex < 0) {
			cout << "      not in any group\n";
			continue;
		}

		const GraphGroup& group = groups[groupIndex];
		cout << "      group " << groupIndex << " graph0 matches ";
		printMatchIndices(group.graphIndices[0]);
		cout << " graph1 matches ";
		printMatchIndices(group.graphIndices[1]);
		cout << "\n";

		if (group.graphIndices[0].empty()) {
			cout << "      no bent match shares this exact tuple";
			int bentMatchesWithSameTuple = 0;
			for (const auto& bentTuple : bentBoundaryValues) {
				if (bentTuple == splicedBoundaryValues[m]) {
					bentMatchesWithSameTuple++;
				}
			}
			cout << " (checked " << bentBoundaryValues.size()
				<< " bent matches, " << bentMatchesWithSameTuple << " equal)\n";
		}
	}
}

void debugFilterDropV15V33(
	const vector<GraphGroup>& groups,
	const vector<TemplateMatcher>& matchers
) {
	for (const GraphGroup& group : groups) {
		int templatesWithMatches = 0;
		int onlyGraph = -1;
		for (int g = 0; g < (int)group.graphIndices.size(); g++) {
			if (!group.graphIndices[g].empty()) {
		...
		if (templatesWithMatches >= 2) {
			continue;
		}
		for (int m : group.graphIndices[1]) {
			if (!matchers[1].isV15V33Match(m)) {
				continue;
			}
			cout << "    filterEmptyGraphGroups: drop v15/v33 match " << m
				<< " in spliced-only group, tuple ";
			printBoundaryTuple(group.boundaryValues);
			if (onlyGraph >= 0) {
				cout << ", only graph " << onlyGraph << " has matches";
			}
			cout << "\n";
		}
	}
...
#ifdef _DEBUG
			debugInspectBendWallMatches(matchers, primitiveGraphs, boundaryIds);
#endif
			auto preFilterGroups = groupGraphs(allBoundaryValues);
			debugV15V33Grouping(
				preFilterGroups, allBoundaryValues, matchers, "after groupGraphs");
			debugFilterDropV15V33(preFilterGroups, matchers);
			auto graphGroups = filterEmptyGraphGroups(preFilterGroups);
			debugV15V33Grouping(
				graphGroups, allBoundaryValues, matchers, "after filterEmptyGraphGroups");
			for (int m = 0; m < (int)matchers[1].vertexValues.size(); m++) {
				if (!matchers[1].isV15V33Match(m)) {
					continue;
				}
				bool inGroup = false;
				for (const auto& group : graphGroups) {
					if (group.graphIndices.size() < 2) {
						continue;
					}
					for (int index : group.graphIndices[1]) {
						if (index == m) {
							inGroup = true;
							break;
						}
				...
				cout << "    v15/v33 match " << m << " (types at 4,5 = "
					<< matchers[1].matchVertexTypeValue(m, 4) << ","
					<< matchers[1].matchVertexTypeValue(m, 5) << "): "
					<< (inGroup ? "exporting" : "dropped by groupGraphs") << "\n";
			}
